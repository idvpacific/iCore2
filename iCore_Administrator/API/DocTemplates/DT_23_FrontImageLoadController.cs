    ...
    public class DT_23_FrontImageLoadController : ApiController
    {
        ...
        public HttpResponseMessage Post(string CID, string SID, string DTID, string DID)
        {
            ...
            try
            {
                ...
                DT = SQ.Get_DTable_TSQL(DataBase_Selector.Administrator, "Select ImageData From Template_07_Images Where (CID = '" + CID + "') And (SID = '" + SID + "') And (DTID = '" + DTID + "') And (DID = '" + DID + "') And (ImageID = '1')");
                if (DT.Rows != null)
                {
                    if (DT.Rows.Count == 1)
                    {
