    ...
    public class ApplicationsController : Controller
    {
        ...
        public JsonResult HSU_Grid()
        {
            try
            {
                ...
                if (ResVal == "0")
                {
                    ...
                    DT = Sq.Get_DTable_TSQL(DataBase_Selector.Administrator, "Select User_FullName,Customer_FullName,Email,TrakingCode,Status_Code,Ins_Date,Last_Update_Date,Application_UID From Users_08_Hospitality_SingleUser_Application_V1 Where (Removed = '0')");
                    if (DT.Rows != null)
                    {
                        foreach (DataRow RW in DT.Rows)
                        {
                            ...
                            ResSTR += "<tr>";
                            ResSTR += "<td onclick=\"HSU_Application_Show('" + RW[7].ToString().Trim() + "')\" style=\"cursor:pointer\">" + RW[0].ToString().Trim() + "</td>";
                    ...
                    {
                        ResVal = "1"; ResSTR = "The server encountered an error while reloading HUS application information";
                    }
                }
        ...
        public JsonResult HSU_New_Grid()
        {
            try
            {
                ...
                {
                    DataTable DT = new DataTable();
                    DT = Sq.Get_DTable_TSQL(DataBase_Selector.Administrator, "Select User_FullName,Customer_FullName,Email,TrakingCode,Status_Code,Ins_Date,Last_Update_Date,Application_UID From Users_08_Hospitality_SingleUser_Application_V1 Where (Seen_Admin_Flag = '0') And (Removed = '0')");
                    if (DT.Rows != null)
                    {
                        foreach (DataRow RW in DT.Rows)
                        {
        ...
        public ActionResult HSU_Application()
        {
            try
            {
                ...
                DT_App = Sq.Get_DTable_TSQL(DataBase_Selector.Administrator, "Select * From Users_08_Hospitality_SingleUser_Application Where (App_UnicID = '" + AppUID + "') And (Removed = '0')");
                if (DT_App.Rows != null)
                {
                    if (DT_App.Rows.Count == 1)
                    {
                        if (DT_App.Rows[0][29].ToString().Trim().ToLower() == "false")
                        {
                            string InsDate = Sq.Sql_Date();
                            string InsTime = Sq.Sql_Time();
                            Sq.Execute_TSql(DataBase_Selector.Administrator, "Update Users_08_Hospitality_SingleUser_Application Set [Seen_Admin_Flag] = '1',[Seen_Admin_ID] = '" + UID + "',[Seen_Admin_Date] = '" + InsDate + "',[Seen_Admin_Time] = '" + InsTime + "' Where (ID = '" + DT_App.Rows[0][0].ToString().Trim() + "')");
                        }
                        ViewBag.DT_App = DT_App.Rows[0];
                        DataTable DT_User = new DataTable();
                        DT_User = Sq.Get_DTable_TSQL(DataBase_Selector.Administrator, "Select * From Users_02_SingleUser Where (ID = '" + DT_App.Rows[0][3].ToString().Trim() + "')");
                    ...
                    {
                        return RedirectToAction("HSU", "Applications", new { id = "", area = "ManagementPortal" });
                    }
                }
                else
                {
        ...
        public JsonResult Application_ChangeStatus(string AID, string SID)
        {
            try
            {
                ...
                if (ResVal == "0")
                {
                    ...
                    string InsDate = Sq.Sql_Date();
                    string InsTime = Sq.Sql_Time();
                    Sq.Execute_TSql(iCore_Administrator.Modules.DataBase_Selector.Administrator, "Update Users_08_Hospitality_SingleUser_Application Set [Last_Update_Date] = '" + InsDate + "',[Last_Update_Time] = '" + InsTime + "',[Last_Update_IDVAdmin_ID] = '" + UID + "',[Status_Code] = '" + SCD + "',[Status_Text] = '" + STT + "' Where (ID = '" + AID + "')");
                }
                IList<SelectListItem> FeedBack = new List<SelectListItem> { new SelectListItem { Value = ResVal, Text = ResSTR.Trim() } };
                return Json(FeedBack, JsonRequestBehavior.AllowGet);
            }
